...
SERVER_NAME='plink'
# jar名称
JAR_NAME='plink-web-0.2.0-SNAPSHOT.jar'
cd `dirname $0`
BIN_DIR=`pwd`
cd ..
DEPLOY_DIR=`pwd`
...
# 获取应用的端口号
SERVER_PORT=`sed -nr '/port: [0-9]+/ s/.*port: +([0-9]+).*/\1/p' config/application-local.yml`
...
CONFIG_FILES="  -Dspring.config.location=$CONF_DIR/application-local.yml "
